...
def build_graph() -> Any:
    ...
    for agent in agents:
        graph.add_node(agent.name, make_agent_subgraph(agent))
...
def run(query: str) -> FinalReport:
    ...
    configure_logging()
    graph = build_graph()
    initial = GraphState(user_input=query, final_summary="", final_markdown="")
...
def state_to_report(state_dict: dict[str, Any]) -> FinalReport:
    ...
    quality_reviews = {
        name: report.metadata["quality_review"]
        for name, report in state_dict.get("reports", {}).items()
        if "quality_review" in report.metadata
    }
    return FinalReport(
        ...
        metadata={"quality_reviews": quality_reviews},
    )
