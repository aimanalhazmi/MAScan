...
def _citation_refs_from_inline_token(token: object) -> list[CitationRef]:
    ...
    while index < len(children):
        ...
        if label.isdigit() and (
            url.startswith(("http://", "https://")) or uploaded_document_name(url)
        ):
            refs.append(
                CitationRef(
                    number=int(label),
...
def uploaded_document_name(url: str) -> str | None:
    """Return the safe decoded filename for a MAScan upload link."""
    try:
        parts = urlsplit(url)
    except ValueError:
        return None
    if parts.scheme or parts.netloc or parts.query or parts.fragment:
        return None
    if not parts.path.startswith(UPLOADED_FILE_PREFIX):
        return None
    document = unquote(parts.path.removeprefix(UPLOADED_FILE_PREFIX))
    if (
        not document
        or document in {".", ".."}
        or "/" in document
