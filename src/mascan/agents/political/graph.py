"""Private LangGraph workflow for PoliticalAgent."""
...
class PoliticalAgentState(BaseModel):
    """State internal to the Political agent's private graph."""
...
def build_political_graph(agent: Any) -> Any:
    """Build the behavior-preserving private graph for PoliticalAgent."""

    def gather_deterministic(state: PoliticalAgentState) -> dict[str, Any]:
        return {"deterministic_outputs": agent.gather_deterministic(state.tasks)}

    def run_react_agent(state: PoliticalAgentState) -> dict[str, Any]:
        findings, llm_used_tools = agent.run_react_agent(
            state.tasks,
    ...
    def collect_sources(state: PoliticalAgentState) -> dict[str, Any]:
        return {
    ...
    def build_report(state: PoliticalAgentState) -> dict[str, Any]:
