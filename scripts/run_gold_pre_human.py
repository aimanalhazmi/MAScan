"""Run or print the pre-human phase of the gold-standard experiment."""
...
from mascan.eval.pipeline import build_pre_human_commands, run_pipeline_commands
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "and build the human calibration packet. Defaults to dry-run."
        )
    )
    ...
    parser.add_argument(
        "--reviewer-out-dir",
        default=None,
        help="Optional directory for per-rater packet/rating files.",
    )
    parser.add_argument(
        "--trace-csv-out",
        default=None,
    ...
    commands = build_pre_human_commands(
        ...
        reviewer_out_dir=args.reviewer_out_dir,
    ...
    if not args.skip_preflight:
        report = run_gold_preflight(manifest, base_dir=args.base_dir, phase="pre_human")
